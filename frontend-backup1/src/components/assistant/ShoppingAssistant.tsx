...
export function ShoppingAssistant() {
  ...
  return <>
    <button id="asistente" onClick={() => setOpen(true)} className="fixed bottom-5 right-5 z-30 rounded-full bg-[#6d4aff] text-white shadow-xl px-4 py-3 flex items-center gap-2 font-semibold text-sm"><MessageCircle size={19}/> <span className="hidden sm:inline">Asistente IA</span></button>
    {open && <div className="fixed inset-0 z-50 bg-black/25 sm:bg-transparent pointer-events-auto" onMouseDown={(e) => { if (e.target === e.currentTarget) setOpen(false); }}>
      <div className="absolute bottom-0 right-0 sm:bottom-5 sm:right-5 w-full sm:w-[390px] h-[78vh] sm:h-[600px] bg-white sm:rounded-2xl shadow-2xl border border-neutral-200 flex flex-col overflow-hidden">
        <div className="px-5 py-4 border-b flex items-center justify-between"><div><div className="flex items-center gap-2 font-bold"><Sparkles size={17} className="text-[#6d4aff]"/>Asistente de compras</div><p className="text-xs text-neutral-500 mt-1">Recomendaciones basadas en tu catálogo</p></div><button onClick={() => setOpen(false)} className="p-2 rounded-full hover:bg-neutral-100"><X size={18}/></button></div>
