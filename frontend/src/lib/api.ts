...
export const api = {
  ...
  async chatAssistant(message: string, history: Array<{ role: 'user' | 'assistant'; text: string }> = []) {
    ...
    let data;
    try {
      data = await fetchJson(`${url}/assistant`, request);
    } catch (error) {
      if ((error as { status?: number }).status !== 404) throw error;
      data = await fetchJson(url, request);
    }
    return {
  ...
  async generateDescription(productId: string, tone = 'elegante') {
    ...
    const data = await fetchJson(`${url}/products/${productId}/describe`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ tone, save: true }),
    });
